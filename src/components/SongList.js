import React from "react";
import "./SongList.css";

function SongList() {
    return(
        <div className="parent_container">
            <div className="songlist_container">
                <section id="fast">
                    <h1>Fast</h1>
                    <ul>
                        <li>Ain't No Stopping Us Now</li>
                        <li>Ain’t Too Proud To Beg</li>
                        <li>All Night Long</li>
                        <li>Apple Bottom Jeans</li>
                        <li>Black Magic Woman</li>
                        <li>​Boogie Shoes</li>
                        <li>Brick House</li>
                        <li>Can’t Get Enough Of Your Love</li>
                        <li>Celebration</li>
                        <li>Cupid Shuffle</li>
                        <li>Disco Inferno</li>
                        <li>Electric Slide</li>
                        <li>First, Last, Everything</li>
                        <li>Fresh</li>
                        <li>Get Down On It</li>
                        <li>Get Down Tonight</li>
                        <li>Happy</li>
                        <li>Hora (Hava Nagila)</li>
                        <li>Hot Hot Hot</li>
                        <li>I Feel Good</li>
                        <li>I Gotta Feeling</li>
                        <li>La Bamba</li>
                        <li>Ladies Night</li>
                        <li>Let's Groove Tonight</li>
                        <li>Love Train</li>
                        <li>Low</li>
                        <li>Mustang Sally</li>
                        <li>Never Too Much</li>
                        <li>Oh What A Night</li>
                        <li>Old Time Rock and Roll</li>
                        <li>Play That Funky Music</li>
                        <li>Proud Mary</li>
                        <li>Rappers Delight</li>
                        <li>Rock Around The Clock</li>
                        <li>September</li>
                        <li>She’s A Bad Mama Jama</li>
                        <li>Shout</li>
                        <li>Soul Man</li>
                        <li>Stroking</li>
                        <li>​Sugar Pie, Honey Bunch</li>
                        <li>The Twist</li>
                        <li>This Is How We Do It</li>
                        <li>Twistin' the Night Away</li>
                        <li>​Uptown Funk</li>
                        <li>What I Say</li>
                        <li>Whenever You Call Me</li>
                        <li>Yeah, Yeah</li>
                        <li>YMCA</li>
                        <li>You Dropped the Bomb on Me</li>
                        <li>You Sexy Thing</li>
                        <li>You're the First, the Last, My Everything</li>
                    </ul>

                </section>
                <section id="medium">
                    <h1>Medium</h1>
                    <ul>
                        <li>Billy Jean</li>
                        <li>Blueberry Hill</li>
                        <li>Broadway</li>
                        <li>Brown Eyed Girl</li>
                        <li>Carolina Girl</li>
                        <li>Chain Gang</li>
                        <li>Cupid</li>
                        <li>Dock of the Bay</li>
                        <li>Don't Look Any Further</li>
                        <li>Drift Away</li>
                        <li>Fly Me To The Moon</li>
                        <li>Get Up, Get On Up</li>
                        <li>Girl From Ipanema</li>
                        <li>​Hold On, I'm Comin'</li>
                        <li>How Sweet It Is</li>
                        <li>I Can See Clearly Now</li>
                        <li>I Got a Woman</li>
                        <li>I Love Beach Music</li>
                        <li>It Had To Be You</li>
                        <li>It's Alright</li>
                        <li>Just the Two of Us</li>
                        <li>Just the Way You Are</li>
                        <li>Kansas City</li>
                        <li>Lady Love</li>
                        <li>Lady Soul</li>
                        <li>​Let's Do the Twist</li>
                        <li>Lets Stay Together</li>
                        <li>Lovely Day</li>
                        <li>L-O-V-E</li>
                        <li>Margaritaville</li>
                        <li>Mercy, Mercy, Me</li>
                        <li>Moondance</li>
                        <li>My Girl</li>
                        <li>Never Too Much</li>
                        <li>Papa Was A Rolling Stone</li>
                        <li>Papa’s Got A Brand New Bag</li>
                        <li>Second That Emotion</li>
                        <li>See You When I Git There</li>
                        <li>She Used To Be My Girl</li>
                        <li>Sixty Minute Man</li>
                        <li>Soul Man</li>
                        <li>Stand By Me</li>
                        <li>Superstitious</li>
                        <li>Tennessee Waltz</li>
                        <li>​Tennessee Whiskey</li>
                        <li>That's The Way, I Like It</li>
                        <li>Theme from Shaft</li>
                        <li>The Way You Look Tonight</li>
                        <li>Twist And Shout</li>
                        <li>Under The Boardwalk</li>
                        <li>Up On The Roof</li>
                        <li>Use Me</li>
                        <li>Walking My Baby Back Home</li>
                        <li>What A Wonderful World</li>
                        <li>What’s Going On</li>
                        <li>What You Won't Do For Love</li>
                        <li>When Doves Cry</li>
                        <li>With This Ring</li>
                        <li>You'll Never Find Another Love Like Mine</li>
                        <li>​Your Man</li>
                    </ul>
                </section>
                <section id="slow">
                    <h1>Slow</h1>
                    <ul>
                        <li>A Change Is Gonna Come</li>
                        <li>Ain’t No Sunshine</li>
                        <li>Always and Forever</li>
                        <li>America the Beautiful</li>
                        <li>At Last</li>
                        <li>Bring It On Home To Me</li>
                        <li>Can't Help Falling In Love</li>
                        <li>Crazy Love</li>
                        <li>Daddy's Little Girl</li>
                        <li>Easy</li>
                        <li>For the Good Times</li>
                        <li>Georgia</li>
                        <li>Great Pretender</li>
                        <li>Have I Told You Lately</li>
                        <li>I Left My Heart in San Francisco</li>
                        <li>I Loved Her First</li>
                        <li>Impossible Dream</li>
                        <li>It's Just a Matter of Time</li>
                        <li>Just My Imagination</li>
                        <li>Kiss and Say Goodbye</li>
                        <li>Lean On Me</li>
                        <li>Let Her Cry</li>
                        <li>Let's Do It Again</li>
                        <li>Let's Get It On</li>
                        <li>New York, New York</li>
                        <li>Oh Baby Baby</li>
                        <li>One In a Million</li>
                        <li>People Get Ready</li>
                        <li>Purple Rain</li>
                        <li>Rainy Night in Georgia</li>
                        <li>Sexual Healing</li>
                        <li>Star Spangled Banner</li>
                        <li>Stuck On You</li>
                        <li>Summertime</li>
                        <li>These Arms</li>
                        <li>Tracks Of My Tears</li>
                        <li>Truly</li>
                        <li>Unchained Melody</li>
                        <li>Wonderful World</li>
                        <li>You Don't Know Me</li>
                        <li>You Raise Me Up</li>
                        <li>You Send Me</li>
                    </ul>
                </section>
                <section id="raggae">
                    <h1>Raggae</h1>
                    <ul>
                        <li>Is This Love?</li>
                        <li>No Woman No Cry</li>
                        <li>One Love</li>
                        <li>Red Red Wine</li>
                        <li>Stir It Up</li>
                    </ul>
                </section>
                
                <section id="jazz_instramentals">
                    <h1>Jazz Instramentals</h1>
                    <ul>
                        <li>All Blues</li>
                        <li>Bags Groove</li>
                        <li>Blue Bossa</li>
                        <li>Breezin'</li>
                        <li>Cameleon</li>
                        <li>Girl From Ipanema</li>
                        <li>Misty</li>
                        <li>Mr. Magic</li>
                        <li>Saint Thomas</li>
                        <li>So What</li>
                        <li>Sugar</li>
                        <li>Watermelon Man</li>
                    </ul>
                </section>
                <section id="fats_domino_show">
                    <h1>Fats Domino Show</h1>
                    <ul>
                        <li>Ain't That a Shame</li>
                        <li>Blue Monday</li>
                        <li>Blueberry Hill</li>
                        <li>I Hear You Knocking</li>
                        <li>I Want to Walk You Home</li>
                        <li>I'm in Love Again</li>
                        <li>I'm Ready</li>
                        <li>I'm Walking</li>
                        <li>Let the Four Winds Blow</li>
                        <li>My Blue Heaven</li>
                        <li>My Girl Josephine</li>
                        <li>Poor Me</li>
                        <li>Shake Rattle and Roll</li>
                        <li>Three Nights a Week</li>
                        <li>Walking to New Orleans</li>
                        <li>Your Cheating Heart</li>
                    </ul>
                </section>
            </div>
            </div>

    );
}
export default SongList;